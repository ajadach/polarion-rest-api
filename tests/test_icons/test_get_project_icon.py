"""
Pytest tests for get_project_icon method in Icons class.

Tests the GET /projects/{projectId}/enumerations/icons/{iconId} endpoint.
Uses mocks to avoid hitting real API.

Run with:
    pytest test_get_project_icon.py -v
"""
import pytest
...
@pytest.mark.get
class TestGetProjectIcon:
    """Unit tests for get_project_icon method using mocks"""
    
    def test_get_project_icon_success_with_200(self, mock_icons_api):
        """Test successful retrieval of a project icon with 200 status code (mocked)"""
        ...
        # Execute
        response = mock_icons_api.get_project_icon(
        ...
        assert 'projects/TEST_PROJECT/enumerations/icons/example.gif' in call_args[0][0]
        print("\n✓ Mock: Project icon retrieved successfully with 200 status code")
    
    def test_get_project_icon_unauthorized_401(self, mock_icons_api):
    ...
    def test_get_project_icon_bad_request_400(self, mock_icons_api):
    ...
    def test_get_project_icon_not_found_404(self, mock_icons_api):
        """Test retrieving non-existent project icon (mocked)"""
        mock_response = Mock()
        ...
        response = mock_icons_api.get_project_icon(
            project_id='TEST_PROJECT',
            icon_id='nonexistent.gif'
        )
        
        # Assert
        assert response.status_code == 404
        print("\n✓ Mock: Non-existent project icon returns 404")
    
    def test_get_project_icon_with_default_fields(self, mock_icons_api):
    ...
    def test_get_project_icon_with_custom_fields(self, mock_icons_api):
    ...
    def test_get_project_icon_with_various_project_ids(self, mock_icons_api):
        ...
        for project_id in project_ids:
            ...
            assert response.status_code == 200
            call_args = mock_icons_api._session.get.call_args
            assert f'projects/{project_id}/enumerations/icons/test.gif' in call_args[0][0]
        
        print("\n✓ Mock: Various project ID formats handled correctly")
    
    def test_get_project_icon_with_various_icon_ids(self, mock_icons_api):
        ...
        for icon_id in icon_ids:
            ...
            assert response.status_code == 200
            call_args = mock_icons_api._session.get.call_args
            assert f'projects/TEST_PROJECT/enumerations/icons/{icon_id}' in call_args[0][0]
        
        print("\n✓ Mock: Various icon ID formats handled correctly")
    
    def test_get_project_icon_endpoint_format(self, mock_icons_api):
        ...
        # Execute (matching CURL parameters)
        response = mock_icons_api.get_project_icon(
        ...
        assert 'projects/projectId/enumerations/icons/iconId' in endpoint
        
        print("\n✓ Mock: Correct project icon endpoint format used")
    
    def test_get_project_icon_response_structure(self, mock_icons_api):
        ...
        mock_response.json.return_value = {
            "data": {
                ...
                "links": {
                    "self": "server-host-name/application-path/projects/TEST_PROJECT/enumerations/icons/example.gif"
                }
            },
            "links": {
                "self": "server-host-name/application-path/projects/TEST_PROJECT/enumerations/icons/example.gif"
    ...
    def test_get_project_icon_with_special_characters(self, mock_icons_api):
        """Test retrieval with IDs containing special characters (mocked)"""
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "data": {
                "type": "icons",
                "id": "project/icon-with_special.chars.gif"
            }
        }
        mock_icons_api._session.get.return_value = mock_response
        
        # Execute with special characters
        project_id = 'Project-With_Special.Chars'
        ...
        assert response.status_code == 200
        call_args = mock_icons_api._session.get.call_args
        assert f'projects/{project_id}/enumerations/icons/{icon_id}' in call_args[0][0]
        print("\n✓ Mock: IDs with special characters handled correctly")
    
    def test_get_project_icon_included_section(self, mock_icons_api):
    ...
    def test_get_project_icon_links_section(self, mock_icons_api):
    ...
    def test_get_project_icon_context_distinction(self, mock_icons_api):
        """Test that project icon uses correct endpoint (different from default/global) (mocked)"""
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "data": {
        ...
        # Verify endpoint uses project context
        assert 'projects/TEST_PROJECT' in endpoint
        assert 'enumerations/icons/test.gif' in endpoint
        assert 'defaulticons' not in endpoint  # Should NOT be default context
        
        print("\n✓ Mock: Project icon endpoint is distinct from default/global contexts")
    
    def test_get_project_icon_project_not_found(self, mock_icons_api):
        """Test retrieving icon from non-existent project (mocked)"""
        mock_response = Mock()
        mock_response.status_code = 404
        mock_response.json.return_value = {
            "errors": [
