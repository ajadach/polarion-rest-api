"""
Pytest tests for get_global_icon method in Icons class.

Tests the GET /enumerations/icons/{iconId} endpoint.
Uses mocks to avoid hitting real API.

Run with:
    pytest test_get_global_icon.py -v
"""
import pytest
...
@pytest.mark.get
class TestGetGlobalIcon:
    """Unit tests for get_global_icon method using mocks"""
    
    def test_get_global_icon_success_with_200(self, mock_icons_api):
        """Test successful retrieval of a global icon with 200 status code (mocked)"""
        ...
        # Execute
        response = mock_icons_api.get_global_icon(icon_id='example.gif')
        ...
        # Verify correct endpoint was called (global context uses /enumerations/icons/)
        call_args = mock_icons_api._session.get.call_args
        assert 'enumerations/icons/example.gif' in call_args[0][0]
        assert 'defaulticons' not in call_args[0][0]  # Should NOT be default context
        print("\n✓ Mock: Global icon retrieved successfully with 200 status code")
    
    def test_get_global_icon_unauthorized_401(self, mock_icons_api):
    ...
    def test_get_global_icon_bad_request_400(self, mock_icons_api):
        ...
        # Execute
        response = mock_icons_api.get_global_icon(icon_id='invalid_icon')
    ...
    def test_get_global_icon_not_found_404(self, mock_icons_api):
        """Test retrieving non-existent global icon (mocked)"""
        mock_response = Mock()
        ...
        # Execute
        response = mock_icons_api.get_global_icon(icon_id='nonexistent.gif')
        
        # Assert
        assert response.status_code == 404
        print("\n✓ Mock: Non-existent global icon returns 404")
    
    def test_get_global_icon_with_default_fields(self, mock_icons_api):
        ...
        response = mock_icons_api.get_global_icon(icon_id='test.gif')
    ...
    def test_get_global_icon_with_custom_fields(self, mock_icons_api):
        ...
        custom_fields = {
            ...
            'enumerations': 'id,attributes.enumName'
        }
        response = mock_icons_api.get_global_icon(
            icon_id='test.gif',
    ...
    def test_get_global_icon_endpoint_distinction(self, mock_icons_api):
        """Test that global icon uses correct endpoint (different from default) (mocked)"""
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "data": {
        ...
        # Execute
        response = mock_icons_api.get_global_icon(icon_id='iconId')
        ...
        # Verify endpoint uses /enumerations/icons/ NOT /enumerations/defaulticons/
        assert 'enumerations/icons/iconId' in endpoint
        assert 'defaulticons' not in endpoint
        
        print("\n✓ Mock: Global icon endpoint is distinct from default icon endpoint")
    
    def test_get_global_icon_with_various_icon_ids(self, mock_icons_api):
        ...
        for icon_id in icon_ids:
            response = mock_icons_api.get_global_icon(icon_id=icon_id)
            
            # Assert
            assert response.status_code == 200
            call_args = mock_icons_api._session.get.call_args
            assert f'enumerations/icons/{icon_id}' in call_args[0][0]
        
        print("\n✓ Mock: Various icon ID formats handled correctly")
    
    def test_get_global_icon_response_structure(self, mock_icons_api):
        ...
        mock_response.json.return_value = {
            "data": {
                ...
                "links": {
                    "self": "server-host-name/application-path/enumerations/icons/example.gif"
                }
            },
            "links": {
                "self": "server-host-name/application-path/enumerations/icons/example.gif"
    ...
    def test_get_global_icon_with_special_characters(self, mock_icons_api):
        ...
        mock_response.json.return_value = {
            "data": {
                "type": "icons",
                "id": "global/icon-with_special.chars.gif"
            }
        }
        mock_icons_api._session.get.return_value = mock_response
    ...
    def test_get_global_icon_included_section(self, mock_icons_api):
    ...
    def test_get_global_icon_links_section(self, mock_icons_api):
    ...
    def test_get_global_icon_context_comparison(self, mock_icons_api):
        """Test to verify global vs default context endpoint difference (mocked)"""
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "data": {
